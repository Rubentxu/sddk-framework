...
8. Return the **release-handoff envelope**: emit `ready_for_release=true` with `{change, branch, merge_policy}`. The orchestrator treats this as a hard obligation: the very next phase is `sdd-kernel-release`, no opt-in. See orchestrator.md § "Release Is Mandatory Post-Archive (v3.3, no opt-out)".
