...
- Research existing capabilities (e.g., from the knowledge vault) BEFORE writing Capabilities — use correct existing names.
...
| Entropy-sdd heuristics | When `recommended_effort ≥ deepen` OR `context_quality ≤ C2` |
...
> CONTRACT with sddk-spec. Research existing capabilities in the knowledge vault before filling in.
...
Execute the `## CLI Contract (sddk ledger)` section of `skills/sddk-propose/SKILL.md` before returning: check `sddk cycle status --root . --scope .`, evaluate the phase gate with `sddk cycle evaluate-gate --outcome passed`, store the proposal artifact (`sddk artifact store --root . --scope . --file {proposal-file} --kind proposal --cycle {cycle_id} --producer sddk-kernel`), and verify with `sddk ledger verify --root . --scope .`. A failed evaluate-gate or store is a BLOCKER — report it in your envelope and stop. Full protocol: `skills/_shared/persistence-contract.md` → CLI Ledger Channel.
## References
