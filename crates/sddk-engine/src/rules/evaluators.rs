        ...
        .map(|rule| {
            ...
            let (status, waiver_id, observed, provenance) = match waiver {
                Some(w) if baseline.ref_.head_anchor <= w.granted_until_sha => (
                    RuleStatus::Waived,
                    Some(w.id.clone()),
                    json!({ "waiver_id": w.id, "reason": w.reason }),
                    None,
                ),
                Some(w) => (
                    RuleStatus::NotApplicable,
                    None,
                    json!({ "phase": "phase0", "rule_id": rule.id }),
                    Some(format!(
...
fn provenance_for(rule_id: &str) -> Option<String> {
    match rule_id {
        "ARCH001" | "ARCH002" | "ARCH003" => {
            Some("ARCH00X detailed evaluator deferred to WI-4 (SDDK2-003.M1)".to_owned())
        }
        "ARCH004" | "ARCH005" => {
            Some("ARCH00X detailed evaluator deferred to WI-5 (SDDK2-003.M5)".to_owned())
        }
