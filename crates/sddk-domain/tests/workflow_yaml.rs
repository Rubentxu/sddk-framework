...
fn test_workflow_yaml_deserialization() {
    ...
    assert!(manifest.phases.contains(&sddk_domain::cycle::Phase::Review));
    assert!(
        manifest
            .phases
    ...
    assert!(transition_ids.contains(&"phase.review.complete"));
    ...
    assert!(manifest.gates.contains_key("review-approved"));
