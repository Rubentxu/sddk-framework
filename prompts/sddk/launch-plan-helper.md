...
| `with_knowledge` | No | boolean | Run knowledge pipeline (scan → verify → import) as preflight. When `with_knowledge: true`, propagate to agents; run scan then verify; import only when `knowledge_approved true AND reviewed plan`. Default: false |
| `knowledge_approved` | No | boolean | Explicit approval for quarantine candidates. Required for import step. When `with_knowledge: true` but `knowledge_approved: false`, import is skipped with "approval required". Quarantine candidates are NEVER auto-imported or auto-approved. Default: false |
