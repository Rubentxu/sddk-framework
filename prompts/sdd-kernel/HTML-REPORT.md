...
Tailwind and Mermaid from CDNs. Editorial, visual, designed for human review and archive.
...
|----------------|------------------|
| `engram` / `none` | `/tmp/sddk-{change-name}-{YYYYMMDD}.html` |
| `openspec` | `openspec/changes/{change-name}/reports/cierre.html` + `/tmp/` copy |
| `hybrid` | Both: `openspec/.../reports/cierre.html` AND `/tmp/` copy |

Resolve temp dir from `$TMPDIR`, fallback to `/tmp`.

After writing the HTML, open it immediately:
- Linux: `xdg-open <path>`
- macOS: `open <path>`
- Windows: `start <path>`
...
## Section Index (16 sections, 4 blocks)

The report is organised in **4 narrative blocks**. Each block answers one question an architect will ask:
...
<html lang="es">
  <head>
    ...
    <script type="module">
      ...
      mermaid.initialize({ startOnLoad: true, theme: "neutral", securityLevel: "loose", flowchart: { curve: "basis", padding: 20 }, sequence: { actorMargin: 60 } });
    </script>
    <style>
...
Gantt-style or waterfall showing phase durations. Mermaid Gantt is best.
...
<section id="pipeline">
  <div class="card p-8">
    ...
    <div class="mermaid">
      gantt
        title SDDK Cycle — {change-name}
        dateFormat  YYYY-MM-DD HH:mm
        axisFormat  %H:%M
        section Plan
        explore        :done, e1, 2026-06-23 09:00, 12m
        propose        :done, p1, after e1, 8m
        spec           :done, s1, after p1, 15m
        design         :done, d1, after p1, 18m
        coherence-1    :done, c1, after s1, 2m
        tasks          :done, t1, after c1, 6m
        section Build
        apply          :done, a1, after t1, 45m
        coherence-2    :done, c2, after a1, 2m
        verify         :done, v1, after c2, 18m
        archive        :active, ar1, after v1, 4m
    </div>
...
The "system in its environment". Shows the change as a black box and its users, dependencies, and external systems. Mermaid C4 syntax.
...
<section id="c4-context">
  <div class="card p-8">
    ...
    <div class="mermaid">
      C4Context
        title Diagrama de contexto — {change-name}

        Person(user, "Usuario final", "Consume la funcionalidad")
        Person(admin, "Operador / Admin", "Gestiona configuración")

        System(system, "{SystemName}", "Sistema modificado por este cambio")
        System_Ext(ext1, "{ExternalSystem1}", "Sistema externo con el que nos integramos")
        SystemDb(db, "{Database}", "Persistencia")

        Rel(user, system, "Usa", "HTTPS")
        Rel(admin, system, "Configura", "HTTPS")
        Rel(system, ext1, "Consulta", "REST/gRPC")
        Rel(system, db, "Lee/Escribe", "TCP")

        UpdateElementStyle(system, $fontColor="white", $bgColor="#1e40af", $borderColor="#1e3a8a")
        UpdateRelStyle(user, system, $textColor="#1e40af", $lineColor="#2563eb")
    </div>
...
If Mermaid C4 syntax isn't available, fall back to `flowchart LR` with the same entities.
    ...
    <div class="mermaid">
      flowchart LR
        ...
        subgraph backend["Backend"]
          API["API REST<br/>(Rust + Axum)"]
          Worker["Async Worker<br/>(Rust + Tokio)"]
        end
        ...
        style API fill:#dbeafe,stroke:#1e40af,color:#0f172a
        style Worker fill:#dbeafe,stroke:#1e40af,color:#0f172a
        style PG fill:#fef3c7,stroke:#92400e,color:#0f172a
        style Redis fill:#fef3c7,stroke:#92400e,color:#0f172a
        style S3 fill:#fef3c7,stroke:#92400e,color:#0f172a
        style Stripe fill:#fce7f3,stroke:#9d174d,color:#0f172a
        style SendGrid fill:#fce7f3,stroke:#9d174d,color:#0f172a
    </div>
    ...
    <div class="mermaid">
      flowchart TB
        subgraph entrada["Capa de entrada"]
          Controller["OrderController<br/>(HTTP handler)"]
        end

        subgraph aplicacion["Capa de aplicación"]
          UseCase["PlaceOrderUseCase<br/>(orquestador)"]
        ...
        subgraph infraestructura["Capa de infraestructura"]
          OrderRepo[("OrderRepository<br/>(port)")]
          PaymentGateway["PaymentGateway<br/>(port)"]
          EmailSender["EmailSender<br/>(port)"]
        end

        subgraph adaptadores["Adaptadores"]
          PostgresAdapter["PostgresOrderRepository"]
        ...
        UseCase --> Validator
        UseCase --> PricingService
        UseCase --> Order
        UseCase -->|"writes"| OrderRepo
        UseCase -->|"calls"| PaymentGateway
        UseCase -->|"notifies"| EmailSender

        Order --> OrderItem
        OrderItem --> Money

        OrderRepo -.->|"implements"| PostgresAdapter
        PaymentGateway -.->|"implements"| StripeAdapter
        EmailSender -.->|"implements"| SendgridAdapter

        classDef entradaCls fill:#dbeafe,stroke:#1e40af
        classDef aplicacionCls fill:#e0e7ff,stroke:#3730a3
        classDef dominioCls fill:#d1fae5,stroke:#065f46
        classDef infraCls fill:#fef3c7,stroke:#92400e
        classDef adapterCls fill:#fce7f3,stroke:#9d174d
        class Controller,UseCase,Validator,PricingService entradaCls
        class Order,OrderItem,Money dominioCls
        class OrderRepo,PaymentGateway,EmailSender infraCls
        class PostgresAdapter,StripeAdapter,SendgridAdapter adapterCls
    </div>
...
<section id="module-map">
  <div class="card p-8">
    ...
    <div class="mermaid">
      flowchart LR
        order_api --> order_domain
        order_api --> order_app
        order_app --> order_domain
        order_app --> payment_port
        order_app --> email_port
        order_app --> pricing_service
        payment_port --> stripe_adapter
        email_port --> sendgrid_adapter
        pricing_service --> product_catalog

        style order_domain fill:#d1fae5,stroke:#065f46
        style payment_port fill:#fef3c7,stroke:#92400e
        style email_port fill:#fef3c7,stroke:#92400e
    </div>
      ...
      <div class="p-4">
        <div class="mermaid">
          sequenceDiagram
            ...
            participant DB as PostgresOrderRepository
            participant PG as PaymentGateway (port)
            participant SP as StripePaymentAdapter
            participant E as EmailSender (port)
            participant SE as SendgridAdapter
            participant Bus as EventBus

            U->>FE: Click "Place order"
            FE->>API: POST /orders {cart}
            API->>UC: execute(cart, customerId)
            UC->>V: validate(cart)
            V-->>UC: ok | errors
            UC->>P: calculate(cart)
            P-->>UC: priced cart
            UC->>D: Order.place(pricedCart)
            D-->>UC: Order + OrderPlaced event
            UC->>R: save(order)
            R->>DB: insert(order)
            DB-->>R: ok
            R-->>UC: persisted
            UC->>PG: charge(customerId, amount)
            PG->>SP: createCharge(token, amount)
            SP-->>PG: chargeId
            PG-->>UC: ChargeResult
            UC->>E: sendConfirmation(order)
            E->>SE: send(to, subject, body)
            SE-->>E: ok
            E-->>UC: ok
...
4. Static analysis (if tools available)
   ...
   → entropy-sdd connascence + SOLID metrics
...
6. Generate Mermaid source code from each data source
   → Use templates above; fill placeholders

7. Render HTML with Mermaid blocks + structured data
...
After writing the HTML, open it:
- Linux: `xdg-open <path>`
- macOS: `open <path>`
- Windows: `start <path>`
...
1. **Sections are conditional**: omit any section whose primary data source is empty or missing. Do not render blank sections.
2. **Mermaid syntax is strict**: validate before saving. Use `flowchart` or `graph` if `C4Context` isn't available in the Mermaid version.
...
6. **Self-contained**: no external assets except CDN-loaded Tailwind, Mermaid, and Google Fonts. Must work offline once loaded.
