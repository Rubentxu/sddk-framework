...
This registry belongs only to the advanced SDD Kernel flow. It is not used by traditional `/sdd-*` commands or `sdd-*` phase agents.
...
Delegators may use `.atl/skill-registry.md` when present. If it is missing, use the known installed skills from the current OpenCode session and the lens rows below.
...
- Delegate: custom heuristic or C4 skill if installed.
- Sources: Simon Brown.
- Evidence: audience, system scope, containers, modules, runtime/deployment relationships.
- Output: recommended C4 level and diagram scope, not full diagram unless requested by phase.
