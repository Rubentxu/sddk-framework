# Strict TDD Module — SDDK Apply Phase
...
Violation of Law 1 = the apply phase MUST fail verification. There is no silent fallback.

## TDD Implementation Cycle (integrates with L3 inner loop)

The TDD cycle plugs into the L3 inner loop's EVALUAR step: each "attempt" in the inner loop is one complete TDD cycle.

```
L3 ATTEMPT N:
├── RAZONAR  (read task + acceptance criteria from spec)
...
│
├── 6. Mark task complete [x] in tasks.md (or mem_update tasks artifact)
└── 7. Note any deviations or issues discovered
```

## Choosing Test Layer

Based on the testing capabilities cached (typically via sddk-init):
...
Detect the test runner from cached testing capabilities:

```
Read test command from (priority order):
...
└── Full suite runs happen in sddk-verify, not here
```
...
## TDD Cycle Evidence (mandatory in apply-progress)
...
If this table is missing or incomplete → sddk-verify WILL reject the work.
...
**Extract-Before-Mock Rule**: If the behavior you want to test is a data transformation, mapping, filtering, or conditional logic, EXTRACT it to a pure function FIRST, then test the pure function directly. No mocks needed.
...
```
L3 attempt N:
  RAZONAR  ← Step 1 (UNDERSTAND) of TDD
  ACTUAR   ← Steps 2-5 (RED + GREEN + TRIANGULATE + REFACTOR) of TDD
  OBSERVAR ← test execution result
  EVALUAR  ← acceptance criteria met? → continue / escalate
```

If Strict TDD Mode is OFF → RAZONAR/ACTUAR/OBSERVAR/EVALUAR still runs, but without RED-first / TRIANGULATE / banned-assertions discipline. The inner loop is the same shape; only the quality gates differ.

If Strict TDD Mode is ON and you complete a task WITHOUT writing tests first → apply fails verification. **There is no silent fallback.**