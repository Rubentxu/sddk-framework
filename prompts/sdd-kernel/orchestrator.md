...
5. Ask/cache artifact store mode: `engram` (default) / `logseq` (if MCP) / `openspec` / `hybrid` / `none`
6. Detect MCP availability: CogniCode, Chronos, LogSeq, cognicode-quality, search providers
7. Run triage (C0-C3 + jurisprudence + capability deployment)
8. Ensure kernel init exists. If missing, launch `sddk-init` first.
...
Once `sdd-kernel-verify` returns `PASS` or `PASS_WITH_WARNINGS`, `sdd-kernel-debt-verify` runs **unconditionally** with depth **derived from path**, not selected by the user. There is no prompt for "debt-verify or direct to archive", and there is no skip option in A-* paths.
...
**Recovery on blocker:** if `sdd-kernel-release` returns `status=blocked`, the orchestrator surfaces the blockers[] and instructs the user to re-run `/sddk-release <change>` (idempotent resume from first uncompleted sub-step). The cycle is not "done" until either `status=success` or the user explicitly aborts.
...
| `cycle-complete` | `sdd-kernel-release` succeeds AND trunk sync passes | Offer validated next-cycle work and one safe-exit action |
